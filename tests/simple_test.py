...
def events_url(events): #функция вывода, для вывода только необходимой информации
	for e in events:
		print(e.title,e.url, e.price, e.category)
...
if __name__ == '__main__':
	try:
		if sys.argv[1]=='db': #посмотреть базу данных на все добавленные события
			...
			cursor = conn.cursor()
			enq="SELECT title,date_start, url, price, availability FROM events;"
			cursor.execute(enq)
		...
		params={'limit':50,'price_max':500,'starts_at_min': "2020-06-23T00:00:00",'starts_at_max': "2020-06-23T23:59:00",
